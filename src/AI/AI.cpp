...
class Type
{
    ...
    int rank;
    int type;
    virtual unsigned short* play_card(card_list yours,unsigned short &size){}//play these cards from player's hand
    virtual int impossible_better(int *num){}//1 - impossible,0 - possible if have bomb, -1 possible if don't have bomb
    virtual int check_type(){return 0;}//check type
    virtual int getlast(){return 0;} //get the largest rank
    virtual int getother(){}//get the rank of other
    virtual int getlength(){}//get a dragon's length
};
...
class One:public Type
{
    public:
        ...
        {
            unsigned short *temp=new unsigned short [1];
            int pur;
            if(rank==12) pur=1;
...
)
{
    ...
    if(rank!=1)
    {
        ...
        else
        {
            ...
            double prob=-1;int chosen_color=0;
            for(int i=3;i>=0;i--)
            {
                if(Smallest_Rank(yours,i)==-1) continue;
                //cout<<"qwq"<<i<<endl;
                if(Count_Larger(vis,i,0)-num[i]==0)
                {
                    prob=19260817;
                    chosen_color=i;
                }
                else if(prob<Count_Larger(vis,i,Largest_Rank(yours,i))/(Count_Larger(vis,i,0)-num[i]))
                {
                    prob=Count_Larger(vis,i,Largest_Rank(yours,i))/(Count_Larger(vis,i,0)-num[i]);
                    chosen_color=i;
                }
            }
            //cout<<chosen_color<<" "<<Smallest_Rank(yours,chosen_color)<<endl;
    ...
    else
    {
        ...
        for(int i=0;i<4;i++)
        {
            if(Smallest_Rank(yours,i)==-1) continue;//cout<<i<<endl;
        ...
        //cout<<prob[0]<<" "<<prob[1]<<" "<<prob[2]<<" "<<prob[3]<<" "<<min_color<<endl;
...
/*int main()
{
    ...
    for(int i=1;i<=10000;i++)
    {
        swap(temp[rand()%52],temp[rand()%52]);
    }
    card_list yours[4];
    card_list hist_cards[55];
    int tot=-1;
    for(int i=0;i<4;i++)
    {
        yours[i].cards=new Card [13];
        yours[i].size=13;
        for(int j=0;j<13;j++)
        {
            yours[i].cards[j]=temp[i*13+j];
        }
    }
    for(int i=0;i<4;i++)
    {
        for(int j=0;j<13;j++)
        cout<<yours[i].cards[j].suit<<" "<<yours[i].cards[j].rank<<endl;
        cout<<endl;
    }
    int rank=0;
    for(int i=0;i<4;i++)
    {
        for(int j=0;j<13;j++)
        {
            if(yours[i].cards[j].suit==0&&yours[i].cards[j].rank==2)
            rank=i;
        }
    }
    for(int i=1;i<=13;i++)
    {
        ...
        for(int j=0;j<=3;j++)
        {
            unsigned short *ans=Play_Hearts(yours[(j+rank)%4],hist_cards,tot+1);
            //cout<<ans[0]<<endl;
            int player=(j+rank)%4;
            cout<<yours[(j+rank)%4].cards[ans[0]].suit<<" "<<yours[(j+rank)%4].cards[ans[0]].rank<<endl;
            tot++;
            yours[(j+rank)%4].size--;
            //cout<<"tot"<<tot<<endl;
            hist_cards[tot].cards=new Card;
            hist_cards[tot].cards[0].suit=yours[(j+rank)%4].cards[ans[0]].suit;
            hist_cards[tot].cards[0].rank=yours[(j+rank)%4].cards[ans[0]].rank;
            for(int l=ans[0];l<yours[(j+rank)%4].size;l++) yours[(j+rank)%4].cards[l]=yours[(j+rank)%4].cards[l+1];
        }
